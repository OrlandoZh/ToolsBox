...
- 默认工作台现已补“项目功能模块图谱 + UI 具象布局图”：会在 `obsidian/agent-workbench/项目模块图谱/` 下按主线 / 支撑层生成模块卡片与 canvas，并额外自动生成 `06-当前扩展波次-模块焦点.md`、`07-能力目录-当前插件能力清单.md` 与 `14-当前Zotero插件-UI 具象布局图.excalidraw.md`
...
- 人工介入：`agent:obsidian` 会把当前 `gate -> monitor -> loop` 结论导出到独立的 `obsidian/agent-workbench/` 工作台目录；默认输出现包括 Markdown 状态页、证据页、Quickstart、Advanced Guide、人工指令窗口、`.canvas` 白板、`项目模块图谱/` 模块卡片、`06-当前扩展波次-模块焦点.md`、`07-能力目录-当前插件能力清单.md` 与 `14-当前Zotero插件-UI 具象布局图.excalidraw.md`，其中状态页/白板会同时展示 `CURRENT_BACKLOG`、project wave、validation override 与自动链状态

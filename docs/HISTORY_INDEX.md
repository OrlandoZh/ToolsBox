...
| 2026-04-20 | decision-record | 路线 4 复用现有后端的快速应用边界 | 明确在“不改后端”的约束下，route4 只能先做 legacy-backend quick apply：Wasm 隐藏客户端协议语义、second-stage only、default-disabled，同时为未来 signed-ticket 蓝图保留升级位 | 保留“现阶段如何快速接入用户验证、哪些边界不能破、何时必须从 legacy 迁到 ticket/签名”的阶段性判断 | [PACKAGE_PROTECTION_ROUTE_PLAN.md](./PACKAGE_PROTECTION_ROUTE_PLAN.md), [ROUTE4_LOCAL_STORAGE_DECISION.md](./ROUTE4_LOCAL_STORAGE_DECISION.md) | [PACKAGE_PROTECTION_ROUTE4_LEGACY_BACKEND_PLAN.md](./PACKAGE_PROTECTION_ROUTE4_LEGACY_BACKEND_PLAN.md) |

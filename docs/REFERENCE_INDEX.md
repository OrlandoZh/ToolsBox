...
| dev-only 浏览器风洞 / 场景模块 / Zotero Web Library 浏览器 UI adapter / Zotero PDF.js 文档分析 harness / Item Tree column resize / 标签体系 / 性能缺口具像化 | [REFERENCE_WIND_TUNNEL_SCENARIO_MODULE_PATTERNS.md](./REFERENCE_WIND_TUNNEL_SCENARIO_MODULE_PATTERNS.md) | [REFERENCE_ZOTERO_WEB_LIBRARY_BROWSER_ADAPTER.md](./REFERENCE_ZOTERO_WEB_LIBRARY_BROWSER_ADAPTER.md), [REFERENCE_ZOTERO_CLIENT_ITEM_TREE_COLUMN_RESIZE.md](./REFERENCE_ZOTERO_CLIENT_ITEM_TREE_COLUMN_RESIZE.md), [REFERENCE_ZOTERO_CLIENT_TAG_SYSTEM.md](./REFERENCE_ZOTERO_CLIENT_TAG_SYSTEM.md), [REFERENCE_KNOW_UR_ZOTERO_PERFORMANCE_PATTERNS.md](./REFERENCE_KNOW_UR_ZOTERO_PERFORMANCE_PATTERNS.md), [REFERENCE_PDF_OVERLAY_POSITIONING_PATTERNS.md](./REFERENCE_PDF_OVERLAY_POSITIONING_PATTERNS.md) | 从相邻模板项目、Zotero 官方 Web Library 与 Zotero 本地端 `VirtualizedTable / TagSelector / tags-box` 抽象 Reader workflow、上下文收敛、浏览器 UI 锚点、官方视觉 token / density / class grammar、Item Tree column resize、Tag Selector / Tags Box、Zotero PDF.js document-analysis harness、文档 overlay、运行时服务、发布更新等通用风洞模块；只做 clean-room 场景化参考，不替代真机 E2E |
...
| PDF overlay 定位 / annotation position / PDF.js viewport 映射 | [REFERENCE_PDF_OVERLAY_POSITIONING_PATTERNS.md](./REFERENCE_PDF_OVERLAY_POSITIONING_PATTERNS.md) | [ZOTERO_HOST_INTERFACE_CONTRACTS.md](./ZOTERO_HOST_INTERFACE_CONTRACTS.md), [REFERENCE_SMART_HIGHLIGHTER_PATTERNS.md](./REFERENCE_SMART_HIGHLIGHTER_PATTERNS.md), [REFERENCE_AI_BAR_PATTERNS.md](./REFERENCE_AI_BAR_PATTERNS.md) | 以 `pageIndex + rects / paths` 为稳定 anchor，经 Reader iframe / PDF.js viewport 转 DOM overlay rect；Smart Highlighter 插值只作 fallback |
| Agent 运行时 / 多轮工具调用 / MCP / HITL 确认 | [REFERENCE_LLM_FOR_ZOTERO_PATTERNS.md](./REFERENCE_LLM_FOR_ZOTERO_PATTERNS.md) | [REFERENCE_PAPER_CHAT_PATTERNS.md](./REFERENCE_PAPER_CHAT_PATTERNS.md) | 先看完整 Agent 架构（5 种模型适配器、10+ HITL 字段、MCP Server、第三方扩展 API），再看 Paper Chat 的轻量 AI 对话实现 |
...
- PDF overlay 定位 / annotation position / PDF.js viewport 映射：[REFERENCE_PDF_OVERLAY_POSITIONING_PATTERNS.md](./REFERENCE_PDF_OVERLAY_POSITIONING_PATTERNS.md)
...
- dev-only 浏览器风洞场景模块：[REFERENCE_WIND_TUNNEL_SCENARIO_MODULE_PATTERNS.md](./REFERENCE_WIND_TUNNEL_SCENARIO_MODULE_PATTERNS.md)
- Zotero Client Item Tree 列宽拖动：[REFERENCE_ZOTERO_CLIENT_ITEM_TREE_COLUMN_RESIZE.md](./REFERENCE_ZOTERO_CLIENT_ITEM_TREE_COLUMN_RESIZE.md)
- Zotero Client 标签体系：[REFERENCE_ZOTERO_CLIENT_TAG_SYSTEM.md](./REFERENCE_ZOTERO_CLIENT_TAG_SYSTEM.md)

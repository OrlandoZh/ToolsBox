...
npm run package:protection:webcrack:shielded:jsconfuser:string -- --channel stable --jsconfuser-tool-path /absolute/path/to/js-confuser
...
补充说明：\`package:encrypted\`、\`package:shielded\`、\`package:shielded:descriptor-bind\` 与 \`package:shielded:jsconfuser:string\` 都只生成本地手动触发的受保护 XPI 分支，不参与默认 release metadata / release gate 主线，也不替代服务端保护；其中 \`package:shielded\` 会先对主 bundle 做混淆，再对 protected loader 做一层兼容性优先的混淆，最后做 AES 包装；\`package:shielded:descriptor-bind\` 则是在此基础上额外挂入一个 host-binding-aware 的 capability descriptor overlay 实验入口，仍保持非阻断、手动实验语义；\`package:shielded:jsconfuser:string\` 则会在现有 protected-only source proxy 上额外跑一层显式目标字符串的 \`JS-Confuser stringConcealing\`，只在你提供外部 \`js-confuser\` checkout 时生效。当前正式手动收口链路固定为 \`package:protection:smoke -> package:protection:score -> package:protection:verdict\`：\`package:protection:smoke\` 只用于手动实验 \`plain / encrypted / shielded\` 三个控制组的安装态、\`packageProtection\` 时序与 base64 fast path 支持情况；\`package:protection:webcrack\` 则会在 fresh 打包后提取 XPI 内脚本、调用本地 \`webcrack\` CLI，并在 \`dist/package-protection-webcrack/\` 下落盘默认首轮与 fallback loader-only 两份自动化工件，供 controller / subagent / 人工继续判读；\`package:protection:webcrack:score\` 会把该工件中的 \`suggestedWebcrackRating\` 回填到 smoke report 的 \`manualScorecard.webcrackInitialResult\`，但不会代替 \`llmSinglePassResult\`；\`package:protection:score\` 仍用于补齐完整手工评分；\`package:protection:verdict\` 只生成 advisory 结论，不进入默认 release 主线。\`package:protection:audit\` 是额外的 raw export 语义泄露实验入口，用来判断下一步应该先 trim loader 还是进入 inner bundle semantic scrub；\`package:protection:audit:descriptor-bind\` 与 \`package:protection:audit:jsconfuser:string\` 则是在不改变上述基线判据的前提下，把额外实验变体纳入同一份审计报告。\`package:protection:jsconfuser:preflight\` 是当前更前置的 Stage 3 候选预检入口，只用于对外部 \`js-confuser\` checkout 做 \`astScrambler\` 的兼容性、体积和语义压缩预检；\`package:protection:jsconfuser:string:preflight\` 则是更激进但仍保持非 hostile 的次级预检入口，只对显式目标字符串做小范围 \`stringConcealing\` 试验；\`package:protection:lightweight:preflight\` 是更后置的 Stage 3 预检入口，只用于对外部 \`lightweight-js-obfuscator\` checkout 做兼容性与体积预检，不会改写默认 \`shielded\` 路线。

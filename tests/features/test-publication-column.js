...
describe("PublicationColumn", () => {
  ...
  it("should cleanup and return stopped resources", () => {
    ...
    assert.ok(cleanupResult.resources[0].includes('publication'), "resource should include column ID");
    assert.equal(unregistered.length, 1, "unregisterColumn should be called");
    assert.equal(unregistered[0], 'publication', "should unregister correct column ID");
  });

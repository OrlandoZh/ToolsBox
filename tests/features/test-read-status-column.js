/**
 * Tests for Read Status Column feature
 */

import { describe, it, assert, runTestsIfMain } from "../test-framework.js";
import { createReadStatusColumn } from "../../src/features/read-status-column.js";
...
describe("ReadStatusColumn", () => {
  it("should register via ItemTreeManager and return true", () => {
    ...
    const column = createReadStatusColumn({
      logger: mockLogger,
      zotero: mockZotero
    });
    ...
    assert.equal(registerArgs.dataKey, 'readStatus', "dataKey should be 'readStatus'");
    assert.equal(registerArgs.pluginID, 'toolsbox@orlandozh.github', "pluginID should match");
    assert.equal(registerArgs.label, 'Read', "label should be 'Read'");
  });
  ...
  it("should use globalThis.Zotero fallback", () => {
    ...
    const column = createReadStatusColumn({ logger: createMockLogger() });
    const result = column.register();
  ...
  it("should handle null/undefined item in dataProvider gracefully", () => {
    ...
    const resultNull = column.dataProvider(null, 'readStatus');
    assert.equal(resultNull, 'false', "dataProvider(null) should return 'false'");

    const resultUndefined = column.dataProvider(undefined, 'readStatus');
    assert.equal(resultUndefined, 'false', "dataProvider(undefined) should return 'false'");
  });
  ...
  it("should render cell element with proper className", () => {
    ...
    const cell = column.renderCell(0, 'true', { className: 'read-status-test-css' });

    assert.ok(cell, "renderCell should return an element");
    assert.ok(cell.className.includes('read-status-test-css'), "cell className should include provided className");
  });
});
